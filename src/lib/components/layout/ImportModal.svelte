...
<Dialog bind:open onOpenChange={handleOpenChange}>
	<DialogContent onCloseAutoFocus={(e) => e.preventDefault()} class="max-w-md p-6 sm:max-w-lg">
		{#if step === 'upload'}
			<DialogHeader class="space-y-1">
				<DialogTitle class="text-xl font-bold">Import Grades from Archers.Hub</DialogTitle>
				<DialogDescription class="text-xs text-muted-foreground">
					This feature is only available for DLSU students via Archers.Hub.
				</DialogDescription>
			</DialogHeader>

			<div class="mt-4 space-y-4 text-xs">
				<ol class="list-decimal space-y-1.5 pl-4 font-medium text-foreground">
					<li>Log in to <span class="font-bold">Archers.Hub</span></li>
				...
				<label
					...
					ondrop={handleDrop}
					class="relative flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-border bg-muted/30 p-6 text-center transition-colors hover:border-primary/50 hover:bg-muted/50"
				>
					<input
					...
					/>
					{#if isParsing}
						<LoaderCircle class="mb-2 size-8 animate-spin text-primary" />
						<p class="font-semibold text-foreground">Parsing grade export...</p>
					{:else}
						<FileSpreadsheet class="mb-2 size-8 text-muted-foreground" />
				...
				<div class="rounded-xl border border-border bg-muted/40 p-3 text-muted-foreground">
					...
					<p class="mt-1.5 leading-relaxed">
						...
						missing entirely due to how Archers.Hub generates this export.
					</p>
		...
		{:else if step === 'confirm'}
			<DialogHeader class="space-y-1">
				<DialogTitle class="text-xl font-bold">Confirm Data Replacement</DialogTitle>
				<DialogDescription class="text-xs text-muted-foreground">
			...
			<div class="mt-4 space-y-4">
				<div class="rounded-xl border border-border bg-card p-4">
					...
					<div
						class="max-h-40 space-y-1 overflow-y-auto pr-1 font-mono text-xs text-muted-foreground"
					>
						{#each parsedTerms as term (term.id)}
							<div class="flex justify-between border-b border-border/40 py-1 last:border-none">
								<span>{term.name}</span>
								<span class="font-semibold text-foreground">{term.courses.length} courses</span>
							</div>
						{/each}
					</div>
				</div>

				<div class="rounded-xl border border-destructive/50 bg-destructive/10 p-4 text-destructive">
				...
				<div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
					<Button variant="ghost" onclick={resetState} size="sm">Cancel</Button>
					<Button variant="destructive" onclick={executeImport} size="sm">
		...
		{:else if step === 'success'}
			<div class="space-y-3 py-4 text-center">
				<div
					class="mx-auto flex size-12 items-center justify-center rounded-full bg-primary/20 text-primary"
				>
					<CircleCheck class="size-6" />
				</div>
				<DialogHeader class="items-center text-center">
					<DialogTitle class="text-xl font-bold">Import Complete</DialogTitle>
				</DialogHeader>
				...
				<Button onclick={() => handleOpenChange(false)} class="mt-2" size="sm">Done</Button>
			</div>
		{/if}
	</DialogContent>
</Dialog>
