    ...
    const colorGroups: ColorGroup[] = [
        {
            ...
            fields: [
                ...
                { id: 'surface', label: 'Surface', cssVar: '--custom-surface', desc: 'Cards, popovers, modals' },
                { id: 'overlay', label: 'Overlay', cssVar: '--custom-overlay', desc: 'Muted structural backgrounds' }
            ]
        },
        {
            title: 'Highlights & Borders',
            fields: [
                { id: 'highlightLow', label: 'Highlight Low', cssVar: '--custom-highlight-low', desc: 'Borders and inputs' },
                { id: 'highlightMed', label: 'Highlight Med', cssVar: '--custom-highlight-med', desc: 'Secondary active states' },
                { id: 'highlightHigh', label: 'Highlight High', cssVar: '--custom-highlight-high', desc: 'Hover states' }
            ]
        },
        {
            title: 'Typography',
            fields: [
                ...
                { id: 'mutedColor', label: 'Muted', cssVar: '--custom-muted-color', desc: 'Disabled or faint text' }
            ]
        },
        {
            title: 'Accents & Brand',
            fields: [
                { id: 'iris', label: 'Iris (Primary)', cssVar: '--custom-iris', desc: 'Primary brand, rings, aurora center' },
                { id: 'rose', label: 'Rose (Accent)', cssVar: '--custom-rose', desc: 'Accent color, secondary aurora glow' },
...
<Dialog.Root bind:open={customDialogOpen}>
    <DropdownMenu.Root>
        <DropdownMenu.Trigger>
            {#snippet child({ props })}
                <Button {...props} variant="ghost" size="icon" class="h-9 w-9 text-muted-foreground hover:text-foreground">
                    <Palette size={18} />
                </Button>
            {/snippet}
        </DropdownMenu.Trigger>
        <DropdownMenu.Content align="end" class="w-48">
            ...
            {#each THEMES as theme (theme.id)}
                <DropdownMenu.Item
                        ...
                        class={$appStore.theme.active === theme.id ? 'bg-primary/10 text-primary font-medium' : ''}
                >
            ...
            <DropdownMenu.Item
                    ...
                    class={$appStore.theme.active === 'custom' ? 'bg-primary/10 text-primary font-medium' : ''}
            >
                <div class="flex items-center justify-between w-full">
    ...
    <Dialog.Content class="sm:max-w-150 max-h-[85vh] overflow-y-auto">
        <Dialog.Header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 py-4">
            {#each colorGroups as group (group.title)}
                <div class="space-y-4">
                    <h4 class="text-sm font-semibold text-foreground border-b border-border pb-1">
                        {group.title}
                    </h4>
                    <div class="space-y-3">
                        {#each group.fields as field (field.id)}
                            <div class="flex items-center justify-between gap-3">
                                <div class="space-y-0.5">
                                    ...
                                    <p class="text-[10px] text-muted-foreground leading-tight max-w-30">
                                        {field.desc}
                                    </p>
                                </div>
                                <div class="flex items-center gap-2">
                                    <div class="relative h-7 w-8 rounded overflow-hidden border border-border shrink-0">
                                        <input
                                                type="color"
                                                id={field.id}
                                    ...
                                    <Input
                                            bind:value={customColors[field.id]}
                                            class="w-20 font-mono uppercase text-[10px] h-7 px-2"
                                    />
                                </div>
                            </div>
                        {/each}
