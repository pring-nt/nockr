		...
		<div class="space-y-4 py-3">
			...
			<div class="relative overflow-hidden rounded-xl border border-border/60 bg-muted/30 p-4">
				...
				<div class="mt-1 flex items-baseline justify-between">
					<span class="gradient-text text-xl font-semibold">
			...
			<div class="relative overflow-hidden rounded-xl border border-primary/20 bg-primary/5 p-4">
				...
				<div class="mt-1 flex items-baseline justify-between">
					...
					<span class="font-mono text-sm text-muted-foreground">
						Max CGPA: <strong class="text-foreground"
							>{projectedCGPA !== null ? projectedCGPA.toFixed(3) : 'N/A'}</strong
						>
					</span>
				</div>
				<p class="mt-2 text-[11px] leading-tight text-muted-foreground">
					Assumes all {remainingUnits} remaining units receive the top grade of
					<span class="font-mono text-foreground">{bestGrade.toFixed(1)}</span>.
				</p>
			</div>
			...
			{#if disqualified}
				<div
					class="flex items-start gap-2.5 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive"
				>
					...
					<div class="space-y-0.5">
						...
						<p class="text-[11px] leading-tight opacity-90">
							One or more graded courses match your university's failing grade ({settings.failingGrade}).
							This may disqualify you from {settings.latinHonorsLabel}. Check with your registrar.
						</p>
					</div>
				</div>
			{/if}
